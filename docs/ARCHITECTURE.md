...
- `conversations` — the **canonical** conversation; `ingress` records where it was *born* (origin metadata, still drives `human_in_loop`). The old `unique(ingress, channel_key)` routing key was **dropped** (multi-surface, migration 0013) — a conversation can now be materialized on more than one surface (§6.6).
- `conversation_surfaces` — **built (migration 0013)** the surface-binding index: a `(surface, external_key)` row binds a Discord post to its conversation; replaces `unique(ingress, channel_key)` as the Discord routing key (§6.6).
- `messages` — `jsonb` content (text, attachments, tool-use/result blocks).
- `message_surface_refs` — **built (migration 0013)** the mirror-idempotency ledger: which messages have been rendered to which surface, so the Discord bot never re-posts one already in a bound post (§6.6).
...
A third channel **`discord-pull`** is the cross-process *pull-to-Discord* seam (multi-surface conversations, §6.6/§9.2). Only the Discord bot can create a forum post, but the trigger — "Continue in Discord" — is an owner action on the **webserver** (`POST /api/conversations/:id/surfaces/discord`), which has no Discord client. So the webserver NOTIFYs `discord-pull` with the bare conversation id as payload; the bot LISTENs it and materializes the post out of band (create + bind + mirror recent history). Fire-and-forget like the `notifications` doorbell — a pull while the bot is down is simply lost and the owner retries (acceptable for an interactive, retryable action).

### 6.3 Job queue (pg-boss)

pg-boss owns its own schema (`pgboss`), treated as a black box. Job payloads are minimal — `type AgentJob = { runId: uuid }` — the `agent_runs` row carries everything else, so a pulled-but-unacked job reconstructs from the run row, migrations don't ripple through payloads, and retries are idempotent (the worker skips already-finished runs). The worker calls `boss.work('agent-run', handler)` with concurrency; because a handler blocks in place while paused for user input (§10.2), `agent-run` jobs use **`retryLimit: 0`** and a **job expiration longer than the max interaction timeout** (§10.4) — a parked worker is never redelivered (no duplicate execution, §7.6), a lost run just fails. Recurring automations are *not* driven by pg-boss `schedule()` (it keys schedules by queue name, which doesn't fit one shared `trigger-detect` consumer); instead `alfred-triggers` owns an in-process cron clock (Croner) that enqueues ordinary `trigger-detect` jobs (DEPLOYMENT §5).
...
### 6.6 Multi-surface conversations (canonical conversation + surface bindings)

**Built (migration 0013; spec `docs/specs/2026-06-21-multi-surface-conversations.md`).** A conversation is **one canonical thread** the owner can start on any surface (voice/typing on web/iOS) and continue on any other — including pulling it into Discord and continuing it there — without it being trapped on the surface it was born on. A conversation's **Discord presence is a separate, explicit, lazily-created materialization**, so any conversation can be pulled into Discord while ephemeral web chats never clutter it.

The model is **canonical conversation + surface binding**. `conversations.ingress` is demoted to **origin metadata** (where it was born; still derives `human_in_loop`), and its old `unique(ingress, channel_key)` routing key is **dropped**. Discord post→conversation resolution moves to a binding table **`conversation_surfaces`** — a `(surface='discord', external_key=<post id>)` row mapping a Discord forum post to its conversation, `unique(surface, external_key)` keeping it 1:1. v1 holds only `surface='discord'` rows (web/iOS/voice resolve by `conversation_id` directly), so it's effectively "the Discord materialization index," not yet a general N-surface registry. Web/iOS need no change — they already aggregate all ingresses for full history.

"Continue in Discord" is **explicit + lazy**: a web button and a Discord `/pull` command. The webserver, lacking a Discord client, hands the request to the bot over the `discord-pull` NOTIFY channel (§6.2); the bot creates a `Conversations`-forum post, writes the binding, mirrors the last ~20 messages in, and opens a **standing LISTEN** so future runs — including web/iOS-originated ones — stream live into the post. **`message_surface_refs`** dedups the mirror (which messages are already in the post), so no separate bridge process is needed. Watcher per-fire conversations fold into this model — a watcher conversation is just one *auto-bound* to Discord (a binding written at materialization, replacing the old channel repoint). The full ingress detail is §9.2 (INGRESSES.md); the one-active-run invariant and approval/question broadcast are unchanged (RUNTIME §7.6/§10.2 — a bound post and the web card both go live, first-responder wins).
...
- `POST /api/conversations/:id/surfaces/discord` — "Continue in Discord": materialize this conversation as a Discord forum post (multi-surface, §6.6/§9.2). The webserver has no Discord client, so it only NOTIFYs the `discord-pull` channel (fire-and-forget) — `202` ack; the bot creates the post + binding out of band. `409` (`alreadyBound`) when the conversation already has a Discord post, `404` if it isn't the owner's.
...
**Moved to `docs/INGRESSES.md`** (§9.2–9.4). All three reuse the §9 ingress contract above; in brief: **Discord** (`alfred-discord`, discord.js — owner-ID-filtered, a **direct-to-Postgres peer ingress** like the webserver: streaming reply edits, Approve/Reject buttons for approvals, native StringSelectMenu + freeform Modal for `ask_user` questions, images both ways, and the chat-command registry surfaced as Discord slash commands via the new `@alfred/commands` package; **now built** — spec `docs/specs/2026-06-16-discord-bot.md`; the **guild-forum conversation model** (a forum post = a conversation, gated on `DISCORD_GUILD_ID`) is also built — spec `docs/specs/2026-06-17-discord-conversation-model.md`; watcher reports now land as **a new forum post per fire**, created on demand from the notifications outbox — spec `docs/specs/2026-06-20-watcher-conversation-per-fire.md`; and **any** conversation can now be **pulled into Discord** (a web button / `/pull` command) and continued there, via the `conversation_surfaces` binding model that replaced `unique(ingress, channel_key)` (multi-surface, §6.6) — spec `docs/specs/2026-06-21-multi-surface-conversations.md`; **Voice** (`alfred-voice`, native-app-only, cloud STT/TTS with server-side keys, on-device wake word — the agent core stays the brain; **built as an I/O modality, not a WS orchestrator**); **Autonomous triggers** (`alfred-triggers`, no human at the other end — scheduled / event-driven / agent-initiated, same enqueue+LISTEN+notify transport, different execution lifecycle per §7.7 / RUNTIME.md; **now built** — the pure scheduler + the tiered detection ladder; each fire gets its own **Alfred-first conversation**, fanned out via the notifications outbox to Web Push **and** the Discord bot). Full detail in `INGRESSES.md`.
...
- **Chat** (`/conversation/:id`) — streams Alfred's turns as one cohesive live block; approvals render *inline* as a card (with a "don't ask again" checkbox that writes the §16 setting); tool calls show as quiet inline chips; image content (uploads, screenshots, generated) renders as thumbnails with a click-to-zoom lightbox; a paperclip control uploads images; the conversation title sits in the header; a slash-command palette offers prefix autocomplete (`/rename`, `/help`); and a footer above the composer shows the conversation's cumulative tokens + cost, climbing live during a run (§6.2 `usage` event).
