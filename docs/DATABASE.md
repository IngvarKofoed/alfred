...
triggers                             -- BUILT (migration 0010) — the watcher definitions, one row
  ...
  conversation_id    uuid fk → conversations.id, nullable  -- persistent watcher conversation (recurring); null for 'self' (carries the originating conv instead). The routing key for a recurring watcher's runs: detect.ts routes escalations through it (when set) and run.ts resolves the watcher by it (getTriggerByConversation). Points at the dedicated 'trigger' conversation by default; the Discord guild-forum model (spec 2026-06-17) repoints it at the watcher's forum-post conversation (ingress='discord').
